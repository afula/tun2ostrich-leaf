...
#[no_mangle]
pub extern "C" fn ostrich_run_with_options(
    rt_id: u16,
...
/// @return ERR_OK on finish running, any other errors means a startup failure.
#[no_mangle]
pub extern "C" fn ostrich_run(rt_id: u16, config_path: *const c_char) -> i32 {
    if let Ok(config_path) = unsafe { CStr::from_ptr(config_path).to_str() } {
...
#[no_mangle]
pub extern "C" fn ostrich_run_with_config_string(rt_id: u16, config: *const c_char) -> i32 {
    if let Ok(config) = unsafe { CStr::from_ptr(config).to_str() } {
        let opts = ostrich::StartOptions {
            config: ostrich::Config::Str(config.to_string()),
            #[cfg(feature = "auto-reload")]
            auto_reload: false,
            runtime_opt: ostrich::RuntimeOption::SingleThread,
        };
...
#[no_mangle]
pub extern "C" fn ostrich_reload(rt_id: u16) -> i32 {
...
#[no_mangle]
pub extern "C" fn ostrich_shutdown(rt_id: u16) -> bool {
...
#[no_mangle]
pub extern "C" fn ostrich_test_config(config_path: *const c_char) -> i32 {
    if let Ok(config_path) = unsafe { CStr::from_ptr(config_path).to_str() } {
