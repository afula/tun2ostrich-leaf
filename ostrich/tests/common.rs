...
// Runs multiple ostrich instances.
pub fn run_ostrich_instances(
    rt: &tokio::runtime::Runtime,
    configs: Vec<String>,
) -> Vec<ostrich::RuntimeId> {
    let mut ostrich_rt_ids = Vec::new();
    let mut rt_id = 0;
...
fn new_socks_outbound(socks_addr: &str, socks_port: u16) -> AnyOutboundHandler {
    // Make use of a socks outbound to initiate a socks request to a ostrich instance.
    let settings = ostrich::config::json::SocksOutboundSettings {
    ...
    let outbounds = vec![ostrich::config::json::Outbound {
        protocol: "socks".to_string(),
        tag: Some("socks".to_string()),
        settings: Some(raw_settings),
    }];
    let mut config = ostrich::config::json::Config {
        log: None,
        inbounds: None,
...
pub fn test_tcp_half_close_on_configs(configs: Vec<String>, socks_addr: &str, socks_port: u16) {
    ...
    let rt = tokio::runtime::Builder::new_current_thread()
        ...
        .build()
        .unwrap();
    let ostrich_rt_ids = run_ostrich_instances(&rt, configs);
    let socks_addr = socks_addr.to_string();
    let res = rt.block_on(rt.spawn(async move {
        tokio::time::sleep(std::time::Duration::from_millis(200)).await;
        let listener = TcpListener::bind("127.0.0.1:3000").await.unwrap();
        let mut sess = ostrich::session::Session::default();
        sess.destination = ostrich::session::SocksAddr::Ip("127.0.0.1:3000".parse().unwrap());
        ...
        assert_eq!(n, 0);
        tokio::time::sleep(
            Duration::from_secs(*ostrich::option::TCP_DOWNLINK_TIMEOUT)
        ...
        assert_eq!(n, 0);
        tokio::time::sleep(
            Duration::from_secs(*ostrich::option::TCP_UPLINK_TIMEOUT)
    ...
    for id in ostrich_rt_ids.into_iter() {
        ostrich::shutdown(id);
        assert!(rt
...
pub fn test_data_transfering_reliability_on_configs(
    ...
    let ostrich_rt_ids = run_ostrich_instances(&rt, configs.clone());
    let mut futs: Vec<ostrich::Runner> = Vec::new();
...
// Runs multiple ostrich instances, thereafter a socks request will be sent to the
...
pub fn test_configs(configs: Vec<String>, socks_addr: &str, socks_port: u16) {
    ...
    let mut bg_tasks: Vec<ostrich::Runner> = Vec::new();
...
async fn wait_for_shutdown(id: ostrich::RuntimeId) {
    loop {
        if !ostrich::is_running(id) {
            return;
        }
